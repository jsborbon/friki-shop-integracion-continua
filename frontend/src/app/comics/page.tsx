...
export default function ComicsPage() {
    ...
    useEffect(() => {
        ...
        fetch(`/api/products?category=COMICS&page=${page}&pageSize=${pageSize}`)
            .then((res) => {
                if (!res.ok) throw new Error("Failed to fetch comics data");
                return res.json();
            })
            ...
            <motion.div layout className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {comicsList.map((comic) => (
                    ...
                    >
                        ...
                        >
                            <Image
                                src={imageSources[comic.id] || "/images/not-found.jpg"}
                                alt={comic.title}
                                fill
                            ...
                            {comic.metadata && (
                                <ul className="mt-2 text-gray-500 dark:text-gray-400 text-sm">
                                    {comic.metadata.issueNumber && (
                                        <li><strong>Issue Number:</strong> {comic.metadata.issueNumber}</li>
                                    )}
                                    {comic.metadata.publisher && (
                                        <li><strong>Publisher:</strong> {comic.metadata.publisher}</li>
                                    )}
                                </ul>
                            )}
                        </div>

      ...
      <motion.div layout className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {moviesList.map((movie) => (
          ...
          >
            ...
            >
              <Image
                src={imageSources[movie.id] || "/images/not-found.jpg"}
                alt={movie.title}
                fill
              ...
              {movie.metadata && (
                <ul className="mt-2 text-gray-500 dark:text-gray-400 text-sm">
                  {movie.metadata.duration && (
                    <li><strong>Duration:</strong> {movie.metadata.duration} min</li>
                  )}
                  {movie.metadata.director && (
                    <li><strong>Director:</strong> {movie.metadata.director}</li>
                  )}
                  {movie.metadata.releaseYear && (
                    <li><strong>Release Year:</strong> {movie.metadata.releaseYear}</li>
                  )}
                </ul>
              )}
            </div>

...
export default function BoardGamesProductPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/products/${params.id}`)
      .then((res) => {
        if (!res.ok) throw new Error("Failed to fetch board game product");
        return res.json();
      })
      .then((data: BoardGamesProduct) => {
        if (data.category !== "BOARD_GAMES") {
          throw new Error("Not a board game product");
        }
        setProduct(data);
        setLoading(false);
      })
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <motion.div className="space-y-6">
          ...
          <ul className="list-disc pl-5">
            {product.metadata?.players && <li><b>Players:</b> {product.metadata.players}</li>}
            {product.metadata?.playTime && <li><b>Play Time:</b> {product.metadata.playTime}</li>}
            {product.metadata?.complexity && <li><b>Complexity:</b> {product.metadata.complexity}</li>}
          </ul>
          {/* Action Buttons */}

...
export default function AnimeProductPage() {
  ...
  const [product, setProduct] = useState<AnimeProduct | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`/api/products/${params.id}`)
      .then((res) => {
        if (!res.ok) throw new Error("Failed to fetch anime product");
        return res.json();
      })
      .then((data: AnimeProduct) => {
        if (data.category !== "ANIME") {
          throw new Error("Not an anime product");
        }
        setProduct(data);
        setLoading(false);
      })
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <motion.div className="space-y-6">
          ...
          <ul className="list-disc pl-5">
            {product.metadata?.regionCode && <li><b>Region Code:</b> {product.metadata.regionCode}</li>}
            {product.metadata?.episodes && <li><b>Episodes:</b> {product.metadata.episodes}</li>}
          </ul>
          {/* Action Buttons */}
